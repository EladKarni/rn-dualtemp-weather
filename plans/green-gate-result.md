...
| 12 | Widget layer English-only; iOS widget hardcodes 24h clock | **Fixed (Android); iOS half corrected 2026-07-28** — widget chrome i18n ×6, day labels via hydrated moment locale, formatDataAge localized, all three time paths honor clockFormat. *Correction:* the "three time paths" in `b5cd4fa` (`WeatherStandard`, `BaseWeatherWidget`, `ForecastRow`) are all Android files — the Swift widget's hardcoded `HH:mm` and English chrome were untouched, so the iOS half of this finding was not actually fixed by that commit. Closed by the 2026-07-28 quality pass (payload v2 + `targets/widget/widgets.swift`; see ios-home-widgets-implementation.md). *Residue (ruled in-spec-out):* `WeatherStandard` "Tap to refresh" footer + root-boundary copy intentionally English | `b5cd4fa` | `widgetDataUtils.formatDataAge.test.ts` (zh assertion), `widgetTaskHandler.test.tsx`, `iosWidgetStorage.test.ts` |
